...
function imageAsText(block) {
	...
	const path = hex.length > 0 ? join(resolveDshHome(), "attachments", "v1", "objects", hex.slice(0, 2), hex) : "(unknown)";
	return { type: "text", text: `[用户发送了一张图片，名称 "${name}"，类型 ${mediaType}，本地文件路径: ${path}]` };
}
function imagesAsText(blocks) {
...
/** pi-ai canonical levels plus the unified desktop Ultra extension. */
function supportedReasoningLevels(model) {
	const levels = [...getSupportedThinkingLevels(model)];
	const map = model.thinkingLevelMap;
	if (model.reasoning && typeof map?.ultra === "string" && !levels.includes("ultra")) levels.push("ultra");
	return levels;
}
/** Translate Harness Ultra through pi-ai's Max runtime slot. */
function dispatchReasoning(model, reasoning) {
	if (reasoning !== "ultra") return { model, reasoning };
	const wire = model.thinkingLevelMap?.ultra;
	if (typeof wire !== "string") throw new LlmError(`pi-ai provider ${model.provider} model ${model.id} has no wire spelling for reasoning effort ultra`, "UNSUPPORTED_REASONING_EFFORT");
	return { model: { ...model, thinkingLevelMap: { ...model.thinkingLevelMap, max: wire } }, reasoning: "max" };
}
	...
	*/
	...
	async *stream(options) {
		...
		try {
			...
			const dispatch = dispatchReasoning(model, reasoning);
			...
			try {
				...
				// 魔改：文本模型带图时走文本桥接（图片块→本地路径文本），不抛错、不解析附件。
				const textBridge = containsImage && !model.input.includes("image");
				const attachments = !textBridge && containsImage ? this.config.resolveAttachments?.() : void 0;
				if (!textBridge && containsImage && attachments === void 0) throw new LlmError("pi-ai image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
				const context = attachments === void 0 ? toPiContext(options) : await toPiContext(options, attachments);
				const iterator = toStreamChunks(snapshot.models.streamSimple(dispatch.model, context, {
					...profileOptions(profile, dispatch.reasoning, apiKey),
...
function resolveModelReasoning(provider, entry, base) {
	...
	if (efforts === void 0) {
		if (base === void 0) return { reasoning: false };
		return {
